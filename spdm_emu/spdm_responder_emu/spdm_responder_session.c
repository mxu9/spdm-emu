    ...
    void *response)
{
    ...
    if (m_use_transport_layer == SOCKET_TRANSPORT_TYPE_VTPM) {
        LIBSPDM_ASSERT(is_app_message);
        status = vtpm_get_response_secured_app_request (
            m_vtpm_context, spdm_context, session_id,
            request, request_size, response, response_size);
    }

    if ((m_use_transport_layer == SOCKET_TRANSPORT_TYPE_PCI_DOE) ||
        (m_use_transport_layer == SOCKET_TRANSPORT_TYPE_VTPM) ||
