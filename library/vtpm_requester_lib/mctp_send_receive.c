...
#include "library/vtpm_requester_lib.h"

/**
 * Send and receive an VTPM message
 *
 ...
 **/
libspdm_return_t vtpm_send_receive_data (const void *mctp_context,
                                         void *spdm_context,
                                         const uint32_t *session_id,
                                         vtpm_message_header_t vtpm_header,
                                         const void *request,
                                         size_t request_size,
                                         void *response,
                                         size_t *response_size)
{
    ...
    uint8_t request_buffer[sizeof(vtpm_message_header_t) + VTPM_MAX_MESSAGE_SIZE];
    vtpm_message_header_t *vtpm_request;
    size_t vtpm_request_size;
    uint8_t response_buffer[sizeof(vtpm_message_header_t) + MCTP_MAX_MESSAGE_SIZE];
